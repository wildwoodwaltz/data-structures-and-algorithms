...
const typeNum = (arr) => {
  let num = arr.filter(element => typeof element === 'number');
  return num;
};
...
const containsAnd = (arr) => {
  let hasAnd = arr.filter(element => element.includes('and'));
  return hasAnd;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let goodValues = arr.filter(num => !forbiddenValues.includes(num));
  return goodValues;
};
