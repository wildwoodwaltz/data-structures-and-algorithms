...
const addValues = (arr) => {
  let sumAllValues = arr.reduce((total, currentNum) => {
...
const returnNames = (arr) => {
  return arr.reduce((charName, char) => {
    charName.push(char.name);
    return charName;
  }, []);
};
...
const reversedString = (str) => {
  return str.split('').reduce((chara, charb) => charb + chara, '');
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((banana, orange) => {
    if (Array.isArray(orange.children)) {
      banana += orange.children.length;
    }
    return banana;
  }, 0);
};
